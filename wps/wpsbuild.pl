...
sub copybackdrop
{
    ...
    if ($backdrop ne '') {
        ...
        my $cmd = "cp $ROOT/$backdrop $rockbox_root/$dst";
        `$cmd`;
    }
}
...
sub copythemefont
{
    ...
    my $o=$font;
    $o =~ s/\.fnt/\.bdf/;
    mkdir "$rockbox_root/fonts";
    my $cmd ="$ROOT/tools/convbdf -f -o \"$rockbox_root/fonts/$font\" \"$ROOT/fonts/$o\" ";
    `$cmd`;
}
...
#        system("cp $dir/$wps .rockbox/wps/");
        ...
        if (-e "$dir/$req_t_wps" ) {
          system("cp $dir/$req_t_wps $rockbox_root/wps/$wps");

        } elsif (-e "$dir/$req_g_wps") {
           system("cp $dir/$req_g_wps $rockbox_root/wps/$wps");
           ...
           if ($#filelist >= 0) {
              if (-e "$dir/$wps_prefix/$req_g") {
                 foreach $file (@filelist) {
                     system("cp $dir/$wps_prefix/$req_g/$file $rockbox_root/wps/$wps_prefix/");
                 }
              }
              elsif (-e "$dir/$wps_prefix") {
                 foreach $file (@filelist) {
                     system("cp $dir/$wps_prefix/$file $rockbox_root/wps/$wps_prefix/");
                 }
              }
              else {
...
;
    ...
    if(-f "$rockbox_root/wps/$cfg") {
        print STDERR "wpsbuild warning: wps/$cfg already exists!\n";
    }
    else {
        open(CFG, ">$rockbox_root/themes/$cfg");
        print CFG @out;
        close(CFG);
    }
}
